...
must_haves:
  truths:
    ...
    - "Zoclo width auto-populates from sum of all bajo module widths in the project"
    - "TYPES array includes cocina entry so it appears in Step 1 project-type picker"
  artifacts:
    - path: "cotizador-template/cotizador-template.html"
      provides: "4 new MODS entries for extras, MODS.*.forType field, step2() filtering + category grouping, zoclo auto-width"
      contains: "forType"
  key_links:
    - from: "MODS entries"
    ...
    - from: "cubierta MODS entry"
      ...
      pattern: "cubierta"
    - from: "addMod('zoclo')"
      to: "bajo module widths sum"
      via: "auto-populate zoclo width from bajo widths"
      pattern: "zoclo.*bajo.*width"
---

<objective>
Add 4 cocina extras module types (zoclo, vista lateral, panel relleno, cubierta) to MODS + calculateParts(), add `forType` category field to all MODS entries, and implement module filtering + category grouping in step2(). Zoclo auto-calculates width from total bajo module widths.
...
<task type="auto">
  ...
  <action>
0. **Verify TYPES includes cocina (CUI-01):** Confirm that TYPES array (around line 3119) already contains `{ id: 'cocina', n: 'Cocina', i: '&#x1f373;' }`. It should already exist from Phase 2 work. If for some reason it is missing, add it. Log confirmation either way.
...
8. **Zoclo auto-width from bajo widths (CEXT-01):** When adding a zoclo module via `APP.addMod('zoclo')`, auto-calculate its width as the sum of all bajo module widths in the current project. In the `addMod` function (or immediately after module creation), add this logic:

```javascript
// Auto-populate zoclo width from total bajo widths
if (type === 'zoclo') {
...
This pre-populates the zoclo width field with the sum of all bajo module widths (in cm). The user can still manually override it. Also display a helper note in step2() below the zoclo width input:
...
Use ES5 syntax only (var, no arrow functions, no template literals). Follow existing AUDIT comment pattern for all dimension constants.
  ...
  <verify>
Search the file for `forType` — every MODS entry should have this field. Search for `zoclo`, `vistaLateral`, `panelRelleno`, `cubierta` in MODS, calculateParts(), and assignHardware(). Search for `cubiertaPrices` in CONFIG. Verify TYPES includes `id: 'cocina'`. Add a zoclo module to a cocina project with 2 bajos (60cm each) and verify zoclo width auto-populates to 120. Confirm no syntax errors by opening the HTML file in a browser (no JS console errors).
  </verify>
  <done>All MODS entries have forType field. 4 extras MODS entries exist with cat:'Extras'. calculateParts() returns pieces for all 4 extras types. cubierta pricing uses m2-based formula from CONFIG. step2() shows cubierta type selector. Zoclo width auto-populates from bajo widths sum. TYPES includes cocina entry.</done>
</task>
...
7. Zoclo auto-populates width from bajo module widths sum
8. TYPES includes cocina entry in Step 1 picker
</verification>
...
- Zoclo width auto-calculated from sum of bajo module widths
- TYPES array contains cocina entry (verified, already exists from Phase 2)
</success_criteria>
